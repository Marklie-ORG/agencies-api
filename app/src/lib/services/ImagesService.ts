...
import type { ImageWithUrl } from "marklie-ts-core/dist/lib/interfaces/OrganizationInterfaces.js";
...
export class ImagesService {
  async saveImage(fileData: any, user: User): Promise<string> {
    ...
    const imageGcsUrl = await gcs.uploadImage(fileData, buffer, destination);
  ...
  async getImages(user: User) {
    const images = await database.em.find(Image, {
      organization: user.activeOrganization!,
    });
  ...
  async deleteImage(uuid: string, user: User) {
    const image = await database.em.findOne(Image, {
      uuid,
      organization: user.activeOrganization!,
    });
    ...
    await gcs.deleteFile(
      image.gsUri.replace("gs://marklie-client-reports/", ""),
    );
